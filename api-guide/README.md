...
| 2022-10-01 | scheduled-for-payout | -28800 | Vipps utbet. 2000023 Vippsnr 57860 | 12345-2000023               |                 | 2022-10-02T00:00:00.000000+0200 |         28800 |            0 |
...
* `reference`:
    * On captures and refunds: This is the unique ID provided by the merchant (or partner) when initiating the payment. This corresponds to the `reference` provided with the ePayment API request (this is called `orderId` in the older eCom API).
    * On `scheduled-for-payout`: The reference that will be put on the bank transfer for the settlement payout  
...
in order to continue on the next *page* of data. Once the end of data is returned,
...
**Please note**: In this example, `hasMore` is true and `cursor` is present.
